...
class EditController extends GetxController {
  ...
  void onInit() {
    editDataGet();
    menuAllImages();
    print("id ${firestoreObj.id} hID = ${firestoreObj.hid}");
    firestoreDocId =firestoreObj.id == "" ? firestoreObj.hid! : firestoreObj.id!;
  ...
  editDataGet() {
    ...
    firestoreObj.businessImage != null
        ...
        : businessImage = "";
    happyHourMenuImage = firestoreObj.menuImage;
    for (var amen in amenityModelList) {
      if (firestoreObj.amenities!.contains(amen.name)) {
        amen.isSelect.value = true;
        amentyAddList.add(amen.name);
      }
    }
    for (var bar in barTypeList) {
      if (firestoreObj.barType!.contains(bar.name)) {
...
//======dailyspecial Update
  dailySpecialUpdate(index) {
    ...
    // print('alldailySpecialList.length: ${alldailySpecialList.length}');
    update();
  }
...
//Location
  ...
  Future uploadHappyHourMenuImage() async {
    menuImageFiles = (await _picker.pickMultiImage(
      imageQuality: 85,
    ))!;
    if (menuImageFiles.isNotEmpty) {
      happyHourMenuImage = menuImageFiles.first!.path;
      if (menuImageFiles.length == 3) {
        menuImageFiles.removeLast();
        happyHourMenuImage = menuImageFiles.first!.path;
  ...
  //   } else {
  //     if (index == 0) {
  //       //eventList[0].key = _formKeys[0];
  //       eventList[index].key?.currentState?.dispose();
  //     }
  //     if (index == 1) {
  //       //eventList[1].key = _formKeys[1];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 2) {
  //       eventList[2].key = _formKeys[2];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 3) {
  //       eventList[3].key = _formKeys[3];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 4) {
  //       eventList[4].key = _formKeys[4];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 5) {
  //       eventList[5].key = _formKeys[5];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 6) {
  //       eventList[6].key = _formKeys[6];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 7) {
  //       eventList[7].key = _formKeys[7];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 8) {
  //       eventList[8].key = _formKeys[8];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 9) {
  //       eventList[9].key = _formKeys[9];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 10) {
  //       eventList[10].key = _formKeys[10];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 11) {
  //       eventList[11].key = _formKeys[11];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 12) {
  //       eventList[12].key = _formKeys[12];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //     if (index == 13) {
  //       eventList[13].key = _formKeys[13];
  //       eventList[index].key?.currentState?.reset();
  //     }
  //   }
  // }
...
//*Business Hour and Days *//
  ...
  //       dayTimeList[index].fromTime = "Select Time";
  //       dayTimeList[index].toTime = "Select Time";
  //       firestoreObj.day?.removeWhere(
  //           (element) => element['Hday'] == dayTimeList[index].day);
  //     }
  //   }
  //   update();
  // }
  ...
  //       firestoreObj.dayLate?.removeWhere(
  //           (element) => element['Hday2'] == dayTimeList2[index].day);
  //       dayTimeList2[index].fromTime2 = "Select Time";
  //       dayTimeList2[index].toTime2 = "Select Time";
  //     }
  //   }
  //   update();
  // }
    ...
    for (var day in dayTimeList) {
      if (day.isSelect.isTrue) {
        ...
        if (addingNewHappyHourFound.isEmpty) {
          ...
          hDayTimeList.add(model);
          firestoreObj.day?.add(model);
        }
      } else {
        hDayTimeList.removeWhere((e) => e['Hday'] == day.day);
    ...
    for (var day in dayTimeList2) {
      if (day.isSelect.isTrue) {
        ...
        if (addingNewHappyHourFound.isEmpty) {
          ...
          hDayTimeLateList.add(model);
          firestoreObj.dayLate?.add(model);
        }
        update();
      } else {
        hDayTimeLateList.removeWhere((e) => e['Hday2'] == day.day);
        dayTimeList2[index].fromTime2 = '';
        dayTimeList2[index].toTime2 = '';
  ...
  ///=====================  copyed from duplicate controller ==============
  ...
  List<LocalFoodModel> lateFoodItems = [];
  ...
  List<LocalDrinkModel> lateDrinkItems = [];
  ...
  String dailySpecialfromtime = "Select Time";
  String dailySpecialtotime = "Select Time";
  ...
  dailySpecialDoneTap() async {
    ...
    if (!dailySpecialKey.currentState!.validate()) {
      ...
      await addToDailyySpecial();
      await updateBusinessHourToFireStore();
      update();
      Get.back();
  ...
  void onEventDoneTap() async {
    ...
    if (eventKey.currentState?.validate() ?? false) {
      await updateBusinessHourToFireStore().then((value) {
        Get.back();
      });
    }
    update();
  ...
  List<Amenities> amenityModelList = [
  ...
  void updateAmenity(index) async {
    amentyAddList = [];
    amenityModelList[index].isSelect.value =
        !amenityModelList[index].isSelect.value;
    for (var e in amenityModelList) {
      if (e.isSelect.isTrue) {
        amenity = amenityModelList[index].name;
        amentyAddList.add(e.name);
      }
    }
  ...
  void updateBartype(index) async {
    ...
    } else if (barTypeList[index].isSelect.isFalse) {
      barTypeAddList.remove(barType);
    } else {
      barTypeAddList.add(barType);
    }
    barTypeList.forEach((element) {
      if (element.isSelect.isFalse) {
        if (barTypeAddList.contains(element.name)) {
          print(element.name);
          barTypeAddList.remove(element.name);
        }
      }
    });
  }
  ...
  Future assignStartEventInEdit() async {
    ...
    Get.toNamed(Routes.editEventsScreen, arguments: firestoreObj);
  ...
  //     selectedEvent
  //         .removeWhere((e) => e['name'] == eventList[index].event.toString());
  //     //eventList[index].isSelect.value = false;
  //     update();
  //   }
  //
  //   // if (eventList[index].isSelect.value == false &&
  ...
  removeDuplicationFromSelectedEvents() {
    ...
    if (amentyAddList.isNotEmpty) {
      Set<String> amenities = {};
      for (int i = 0; i < amentyAddList.length; i++) {
        amenities.add(amentyAddList[i]);
      }
      amentyAddList = [];
      amentyAddList.addAll(amenities.toList());
    }
  }
  ...
  Future<void> updateBusinessHourToFireStore() async {
    ...
    firestoreObj.dailySpecils = alldailySpecialList;
    ...
    Map<String, dynamic> data =
      ...
      dailySpecial: firestoreObj.dailySpecils,
      foodNames: foodList
          .map((e) => {
                "foodname": e.name,
    ...
    menuImageList = [];
    menuImageList.add(obj.menuImage);
    menuImageList.add(obj.menuImage2);
  ...
  Future removeItemsWithTimes() async {
    if (hDayTimeList.isEmpty) {
      for (int i = 0; i < foodList.length; i++) {
        foodList[i].earlyFood.value = false;
      }
      for (int i = 0; i < localdrinkList.length; i++) {
        localdrinkList[i].earlyDrink.value = false;
      }
      earlyFoodItems = [];
      earlyDrinkItems = [];
    }
    if (hDayTimeLateList.isEmpty) {
      for (int i = 0; i < foodList.length; i++) {
        foodList[i].lateFood.value = false;
      }
      for (int i = 0; i < localdrinkList.length; i++) {
        localdrinkList[i].lateDrink.value = false;
      }
      lateFoodItems = [];
      lateDrinkItems = [];
    }
  ...
  onDayTimeDoneTap() async {
    if (showLate || dayTimeList.isNotEmpty) {
      ...
      if (businessKey.currentState!.validate()) {
        ...
        await removeItemsWithTimes().then((value) {
          updateBusinessHourToFireStore();
        });
  ...
  void moveEditFoodItems() {
    Get.toNamed(Routes.editFoodItemScreen, arguments: firestoreObj);
  }

  void removeMenuImage(int i) {
    if (menuImageList.isNotEmpty) {
      menuImageList.removeAt(i);
      update();
    }

    if (menuImageFiles.isEmpty && menuImageList.isEmpty) {
      happyHourMenuImage = '';
    }
    update();
  }

  void removeMenuImageFromFiles(int i) {
    if (menuImageFiles.isNotEmpty) {
      menuImageFiles.removeAt(i);
      update();
    }
    if (menuImageFiles.isEmpty && menuImageList.isEmpty) {
      happyHourMenuImage = '';
