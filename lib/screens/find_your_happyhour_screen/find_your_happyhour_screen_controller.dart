...
class FindYourHappyHourScreenController extends GetxController {
  ...
  fetchHours() async {
    ...
    if (currentLoc != null) {
      _addHappyHourProvider
        ...
        for (var ele in _hoursList) {
          print("*********${ele.businessName}**********");
        }
        newFilter();
  ...
  int parseStartHour(dayTime) {
    return int.parse(dayTime['HfromTime'].toString() != ""
        ? dayTime['HfromTime'].toString().split(':').first
        : "0");
  }
  ...
  filterStartingWithinHour() {
    ...
    // List<String> hourEndDuration = [];
    Set<String> times = {};
    Set<String> endTimes = {};
    Set hourIndicies = {};
    Set endHourIndicies = {};
    Set<String> dailytimes = {};
    Set<String> dailyEndTimes = {};
    Set<String> daytimes = {};
    Set<String> dayEndTimes = {};
    Set<String> dayLateTimes = {};
    Set<String> dayLateEndTimes = {};
        ...
        for (int active = 0;
          ...
          if (dayOfWeek.toLowerCase() ==
              _hoursList[hourIndex]
                  ...
                  .toString()
                  .toLowerCase()) {
            dailyEndTimes.add(_hoursList[hourIndex]
                .dailySpecils![active]['toTime']
                .toString()
                .trim()
                .toLowerCase());
          }
        }
        if (dailyEndTimes.isNotEmpty) {
          endTimes.addAll(dailyEndTimes);
        }
      }
      ...
      if (_hoursList[hourIndex].day != null &&
        ...
        for (int active = 0;
          ...
          if (_hoursList[hourIndex]
                  ...
                  .toLowerCase() ==
              dayOfWeek.toLowerCase()) {
            dayEndTimes.add(_hoursList[hourIndex]
                .day![active]['HtoTime']
                .toString()
                .trim()
                .toLowerCase());
        ...
        if (dayEndTimes.isNotEmpty) {
          times.addAll(dayEndTimes);
        }
      }
      ...
      if (_hoursList[hourIndex].dayLate != null &&
        ...
        for (int active = 0;
          ...
          if (_hoursList[hourIndex]
                  ...
                  .toLowerCase() ==
              dayOfWeek.toLowerCase()) {
            dayLateEndTimes.add(_hoursList[hourIndex]
                .dayLate![active]['HtoTime2']
                .toString()
                .trim()
                .toLowerCase());
          }
        }
        if (dayLateEndTimes.isNotEmpty) {
          endTimes.addAll(dayLateEndTimes);
        }
      }
      for (var element in endTimes) {
        print("end $element");
      }
      print(hourMaps.length);
      hourMaps.add(MapModel(
          hourIndex: hourIndex,
          times: endTimes.map((e) => e.toString().toLowerCase()).toList()));
      print(hourMaps.length);
      hourMaps.add(MapModel(
          hourIndex: hourIndex,
    ...
    for (var hourMap in hourMaps) {
      ...
      // print("hours ::::::::::::::::::::::::::: ${hourMap.times.toString()}");
    }
  ...
  getTodayHappyHour() => box.read("todayHH");
  setTodayHappyHour(HappyHourEndTime h) async {
    List<HappyHourEndTime> data = [];
    List<HappyHourEndTime> hh = await getTodayHappyHour();
    hh.forEach((element) {
      print("local storage data $element");
    });
    // if (hh != null) {
    //   data = (json.decode(hh) as List)
    //       .map((data) => HappyHourEndTime.fromJson(data))
    //       .toList();
    //   data.add(h);
    // }
    box.write("todayHH", data);
  }

  Future<void> printBusinessNames() async {
    try {
      // Reference to the Firestore collection
      CollectionReference happyHourCollection =
          FirebaseFirestore.instance.collection('happyhours');

      // Query the collection and get the documents
      QuerySnapshot querySnapshot = await happyHourCollection.get();

      // Iterate over the documents and print the businessName
      ...
      print('Error printing business names: $e');
    }
  }
  ...
  DateTime createDateTime(String input) {
    DateTime now = DateTime.now();

    DateTime dateTimeAM = DateFormat('h:mm a').parse(input.toUpperCase());
    DateTime combinedDateTimeAM = DateTime(
        now.year, now.month, now.day, dateTimeAM.hour, dateTimeAM.minute);
    return combinedDateTimeAM;
  }
}

class HappyHourEndTime {
  String? hhId;
  String? hhEndTime;
  HappyHourEndTime({this.hhId, this.hhEndTime});

  HappyHourEndTime.fromJson(Map<String, dynamic> json) {
    hhId = json['hhId'];
    hhEndTime = json['hhEndTime'] ?? "";
  }

  Map<String, dynamic> toJson() {
    final Map<String, dynamic> data = <String, dynamic>{};
    data['hhId'] = hhId ?? "";
    data['hhEndTime'] = hhEndTime ?? "";
    return data;
  }
}
